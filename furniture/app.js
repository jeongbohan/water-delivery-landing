(() => {
  ...
  if(canSubmit){button.disabled=false;button.textContent=idleCopy}if(qaMode)document.querySelector('#draftBanner').textContent='검수 모드 · 제출 시 성과에서 제외됩니다';
  ...
  window.addEventListener('message',event=>{if(!/^https:\/\/[a-z0-9-]+-script\.googleusercontent\.com$/.test(event.origin)&&event.origin!=='https://script.google.com'&&event.origin!=='https://script.googleusercontent.com')return;const data=event.data;if(!pending||!data||data.source!=='deliveryin-furniture-direct-form'||data.request_id!==pending)return;if(event.origin==='https://script.google.com'&&event.source!==frame.contentWindow)return;clearTimeout(timeout);if(!data.ok){retry(data.error||'접수 확인이 지연되고 있어요. 다시 시도해주세요.');return}if(!data.lead_id||data.ignored||data.is_test!==qaMode){retry('접수 확인값이 일치하지 않아요. 잠시 후 다시 시도해주세요.');return}pending='';done=true;if(!qaMode&&data.kind!=='duplicate'&&!data.performance_excluded&&data.conversion_eligible){const eventKey=`furniture_lead_event_${data.lead_id}`;if(localStorage.getItem(eventKey)!=='1'){analytics('generate_lead',{submission_kind:'new',event_id:data.lead_id});localStorage.setItem(eventKey,'1')}}show(qaMode?'검수 제출을 기록했어요. 성과 집계에서 제외됩니다.':'1차 지원이 접수됐어요. 담당자가 순차적으로 연락드릴게요.',true);for(const el of form.querySelectorAll('input:not([type=hidden])')){if(el.type==='checkbox'||el.type==='radio')el.checked=false;else el.value=''}region.value='';addressDisplay.querySelector('span').textContent='거주지역을 선택해주세요';addressDisplay.classList.remove('is-selected');button.disabled=true;button.textContent='접수 완료';notice.dataset.leadId=data.lead_id;notice.dataset.downstreamStatus=data.downstream_status||'pending'});
